  ...
  return (
    <SelfEvaluationExercise
      generator={() => {
        ...
        let c = null
        let d = null
        const mode = randomItemFromArray(['pure', 'translate-x', 'translate-y'])
        if (mode === 'translate-x') {
          c = randomItemFromArray([5, 4, 3, 2, 1, -1, -2, -3, -4, -5])
        }
        if (mode === 'translate-y') {
          d = randomItemFromArray([5, 4, 3, 2, 1, -1, -2, -3, -4, -5])
        }
        return { n, k, c, d }
      }}
      renderTask={(data) => (
      ...
      renderSolution={(data) => {
        ...
        if (data.n === 2) {
          return (
            <>
              Der Graph ist eine nach {data.k > 0 ? 'oben' : 'unten'} geöffnete
              Parabel mit dem Öffnungsfaktor {data.k.toLocaleString('de-De')}{' '}
              und dem Scheitel S ( {-(data.c ?? 0)} | {data.d ?? 0} ).
            </>
          )
        }
        if (data.n < 0) {
