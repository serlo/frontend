...
import { FooterLink, HeaderLinkData } from '@/data-types'
...
export function FooterNavNew() {
  ...
  return (
    ...
    >
      ...
      {/* first row */}
      <FooterNavChildren links={footerData.footerNavigation[0].children} />
      {/* subjects */}
      {headerData[0].children && (
        <FooterNavChildren links={headerData[0].children} />
      )}
      {/* newsletter/github */}
      <FooterNavChildren links={footerData.footerNavigation[1].children} />
    </nav>
  )
}

function FooterNavChildren({
  links,
}: {
  links: FooterLink[] | HeaderLinkData[]
}) {
  return (
